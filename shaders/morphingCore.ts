export const morphingCoreShader = {
  vertexShader: `
    uniform float uTime;
    ...
    void main() {
      ...
      // React to mouse displacement
      float distToMouse = length(position.xy - uMouse.xy * 2.0);
      float mouseInfluence = smoothstep(1.5, 0.0, distToMouse) * 0.3;
      
      vNoise = noise + mouseInfluence;
      vec3 newPosition = position + normal * (vNoise * 0.6);
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Mix gradient colors based on noise and normal height
      float colorMix = clamp(vNoise * 1.5 + 0.5, 0.0, 1.0);
      vec3 baseColor = mix(uColorA, uColorB, colorMix);
      vec3 finalColor = mix(baseColor, uColorC, fresnel);
      
      // Add intense glow on edges
      finalColor += uColorA * fresnel * 1.8;
